  ...
  switch (parseInt(messagingStep)) {
    ...
    case 2:
      ...
      sql.connect(config, function(err, connection) {
        ...
        const checkIfExistsQuery = "SELECT TOP 1 * FROM members_tb WHERE phoneNumber = @phoneNumberDescription AND isActive = 1 AND status = 'isCheckingAccount' AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberDescription)";
