...
function updatePensionMessagingStep(statusIsPension, phoneNumberPension, messagingStepPension, textMessageProduct, config, textIDATPension) {
  ...
  SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberPension )`;
    ...
    request.input('messagingStepPension', sql.Int, messagingStepPension);
    ...
    request.query(updateAccounts1, function (err, results) {
      ...
      request.query("SELECT TOP 1 * FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberAmount AND status = @statusServices AND isActive = 1 AND text_id_AT = @textIDAT order by time DESC", function (err, productsResults) {
        ...
        if (productsResults.recordset.length > 0) {
          ...
          var args = {
            data: { product: product, productDescription: productDescription, firstname: firstname, lastname: lastname, email: email, national_ID: national_ID, phoneNumber: phoneNumberAmount, methodOfPayment: methodOfPayment, modeOfPayment: modeOfPayment, amount: amount},
            headers: { "Content-Type": "application/json" }
          };
          console.log(args);
          addNewClient.post("https://api.octagonafrica.com/v1/user/onboardnewclients", args, function (data, response) {
            if ([200].includes(response.statusCode)) {
              ...
              sql.connect(config, function (err) {
                ...
                console.log('Connected to the database');
                const request = new sql.Request();
                const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isProducts', messagingStep= '100', isActive= '100' WHERE phoneNumber = @phoneNumberAmount AND text_id_AT = @textIDAT AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberAmount)`;
                request.input('phoneNumberAmount', sql.NVarChar, phoneNumberAmount);
                ...
                request.query(updateAccounts, function (err, results) {
                  ...
                  sms.sendPremium({
                    to: sender,
                    from: '24123',
                    message: 'Dear '+ firstname + ' ' + lastname + ' , Your request has been successfully received. Its currently being reviewed by our team. Once approved, you will receive an notification on email or sms',
                    bulkSMSMode: 0,
                    keyword: 'pension',
                    linkId: LinkID
                  });
...
module.exports = {updatePensionMessagingStep, updateProductDescription, updateFname, updateLastname, updateEmail, updateNationalID, updateMethodOfPayment, updateModeOfPayment, updateAmount};