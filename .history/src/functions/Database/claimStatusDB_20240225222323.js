              ...
              getClaimStatus.get("https://api.octagonafrica.com/v1/claims/getclaims", args, function (data, response) {
                if ([200].includes(response.statusCode)) {
                  ...
                  sql.connect(config, function (err) {
                    console.log('Connected to the database');
                    const request = new sql.Request();
                    const statusSucces = "ischeckingClaimSuccess";
                    // const messagingPeriodsEntry = "0";
                    const phoneNumberPeriodsEntry = sender;
                    const textIDATPeriodsEntry = textIDAT;

                    const updateAllPeriods = `UPDATE two_way_sms_tb SET status = @statusSucces  ,messagingStep= '100', isActive = 100  WHERE phoneNumber = @phoneNumberPeriodsEntry AND text_id_AT =@textIDATPeriodsEntry AND time = (
                             SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberPeriodsEntry )`;
                    request.input('statusSucces', sql.VarChar, statusSucces);
