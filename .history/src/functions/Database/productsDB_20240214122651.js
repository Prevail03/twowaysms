...
function updatePensionMessagingStep(statusIsPension, phoneNumberPension, meessagingStepPension, texMessageProduct, config, textIDATRatePension)
updatePensionMessagingStep(statusIsPension, phoneNumberPension, messagingStepPension, textMessageProduct, config, textIDATPension); {
  sql.connect(config, function (err) {
    ...
    const updateDelete = `UPDATE two_way_sms_tb SET status = @statusIsPension, messagingStep = @messagingStepPension, rateValue = @texMessageProduct WHERE phoneNumber = @phoneNumberPension AND text_id_AT = @textIDATRatePension AND time = (
  SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberPension )`;
    ...
    request.input('textIDATRatePension', sql.NVarChar, textIDATRatePension);
