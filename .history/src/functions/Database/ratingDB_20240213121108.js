          ...
          addNewUserRating.post("https://api.octagonafrica.com/v1/adduserratings", args, function (data, response) {
            ...
            if ([200].includes(response.statusCode)) {
              ...
              sql.connect(config, function (err) {
                ...
                console.log('Connected to the database');
                const request = new sql.Request();
                const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isRating', messagingStep= '100', isActive= '100'WHERE phoneNumber = @phoneNumberperiodID AND text_id_AT = @textIDATperiodID AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberperiodID)`;
