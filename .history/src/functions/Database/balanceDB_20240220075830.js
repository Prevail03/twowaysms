...
function updateDescription(phoneNumberDescription, textDescription, textIDATDescription, sender, config, textIDAT, sms, balance, LinkID, memberID) {
  sql.connect(config, function (err) {
    ...
    const request = new sql.Request();
    const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isBalance', messagingStep= '2', description = @textDescription, memberID = @memberID WHERE phoneNumber = @phoneNumberDescription AND text_id_AT = @textIDATDescription AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberDescription)`;
          ...
          getBalance.get("https://api.octagonafrica.com/v1/accounts/balance", args, function (data, response) {
            if ([200].includes(response.statusCode)) {
              ...
              sms.sendPremium({
                to: sender,
                from: '24123',
                message: "Dear"+ +",  your  balance for account "+ description+ " is "+ accountBalance,
                bulkSMSMode: 0, 
                keyword: 'pension',
                linkId: LinkID
              });
