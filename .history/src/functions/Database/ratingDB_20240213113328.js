...
function updateRatingValue(statusRateValue, phoneNumberRateValue, messagingRateValue, textRateValue, config, textIDATRateValue) {
  sql.connect(config, function (err) {
    ...
    const updateDelete = `UPDATE two_way_sms_tb SET status = @statusRateValue, messagingStep = @messagingRateValue, user_username = @textRateValue WHERE phoneNumber = @phoneNumberRateValue AND text_id_AT = @textIDATUserName AND time = (
