...
function updateAmount(sender, statusAmount, phoneNumberAmount, messagingStepAmount, textAmount, config, textIDATAmount, textIDAT, sms, LinkID) {
  sql.connect(config, function (err) {
    ...
    const request = new sql.Request();
    const updateAccounts1 = `UPDATE two_way_sms_tb SET status = @statusAmount, messagingStep= @messagingStepReason, ratingReason = @textReason WHERE phoneNumber = @phoneNumberReason AND text_id_AT = @textIDATReason AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberReason)`;
