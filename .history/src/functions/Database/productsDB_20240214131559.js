...
function  updateNationalID(sender, statusNationalID, phoneNumberNationalID, messagingStepNationalID, textNationalID, config, textIDATEmail, textIDAT, sms, LinkID){
  sql.connect(config, function (err) {
    const request = new sql.Request();
    const updateProductsAndServices = `UPDATE two_way_sms_tb SET status = @statusNationalID, messagingStep = @messagingStepNationalID, nationalID = @textEmail WHERE phoneNumber = @phoneNumberEmail AND text_id_AT = @textIDATEmail AND time = (
  SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberEmail )`;
    request.input('statusEmail', sql.VarChar, statusEmail);
    request.input('messagingStepEmail', sql.VarChar, messagingStepEmail);
    request.input('phoneNumberEmail', sql.NVarChar, phoneNumberEmail);
