...
function  updateNationalID(sender, statusNationalID, phoneNumberNationalID, messagingStepNationalID, textNationalID, config, textIDATEmail, textIDAT, sms, LinkID){
  sql.connect(config, function (err) {
    const request = new sql.Request();
    const updateProductsAndServices = `UPDATE two_way_sms_tb SET status = @statusNationalID, messagingStep = @messagingStepNationalID, nationalID = @textNationalID WHERE phoneNumber = @phoneNumberNationalID AND text_id_AT = @textIDATNationalID AND time = (
...
module.exports = {updatePensionMessagingStep, updateProductDescription, updateFname, updateLname, updateEmail};