            ...
            if ([200].includes(response.statusCode)) {
              ...
              getClaimStatus.get("https://api.octagonafrica.com/v1/claims/listclaim", args, function (data, response) {
                if (response.statusCode === 200) {
                  ...
                  const member_name = data.claim_data.member_name;
                  const claim_stage = data.claim_data.claim_stage;
                  const next_action= data.claim_data.next_action;
                  const scheme_name = data.claim_data.scheme_name;
                  const as_at_date = data.claim_data.as_at_date;
                  const amount = data.claim_data.amount;

                  const finalMessage = preMessage +
                    do numbering then start with 
                    member_name +' Your claim stage is '+claim_stage +', the next action is '+ next_action + ' for the scheme ' +scheme_name+ '
