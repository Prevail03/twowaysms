    ...
    request.query(updateAccounts, function (err, results) {
      ...
      request.query("SELECT TOP 1 * FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberDescription AND status = @statusDescription AND isActive = 1 AND text_id_AT = @textIDATDescription1 order by time DESC", function (err, descriptionResults) {
        ...
        if (descriptionResults.recordset.length > 0) {
          ...
          request3.query("SELECT TOP 1 * FROM members_tb where m_id = @member_id ", function (err, claimStatusResult) {
            if (err) {
              console.error('Error executing query: ' + err.stack);
              sql.close();
              return;
            ...
            if (claimStatusResult.recordset.length > 0) {
              // const member_email = claimStatusResult.recordset[0].m_email;
              const member_name = claimStatusResult.recordset[0].m_name;
              const member_number = claimStatusResult.recordset[0].m_number;
              const scheme_code = claimStatusResult.recordset[0].m_scheme_code;
