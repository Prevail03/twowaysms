...
function updateDescription(phoneNumberDescription, textDescription, textIDATDescription, sender, config, textIDAT, sms, claimStatus, LinkID, memberID) {
  sql.connect(config, function (err) {
    ...
    request.query(updateAccounts, function (err, results) {
      ...
      request.query("SELECT TOP 1 m.*, t.* FROM two_way_sms_tb t JOIN members_tb m ON CAST(SUBSTRING(t.memberID, CHARINDEX('.', t.memberID) + 1, LEN(t.memberID)) AS INT) = m.m_id WHERE t.phoneNumber = @phoneNumberAmount AND t.status = @statusServices AND t.isActive = 1 AND t.text_id_AT = @textIDATDescription1 ORDER BY t.time DESC;", function (err, descriptionResults) {
        if (err) {
          console.error('Error executing query: ' + err.stack);
          return;
        }
