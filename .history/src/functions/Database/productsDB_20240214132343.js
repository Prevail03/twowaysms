...
function  updateMethodOfPayment(sender, statusNMethodOfPayment, phoneNumberNMethodOfPayment, messagingStepMethodOfPayment, textMethodOfPayment, config, textIDATMethodOfPayment, textIDAT, sms, LinkID){
  sql.connect(config, function (err) {
    const request = new sql.Request();
    const updateProductsAndServices = `UPDATE two_way_sms_tb SET status = @statusMethodOfPayment, messagingStep = @messagingStepMethodOfPayment, methodOfPayment = @textNationalID WHERE phoneNumber = @phoneNumberNationalID AND text_id_AT = @textIDATNationalID AND time = (
  SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberNationalID )`;
    request.input('statusNationalID', sql.VarChar, statusNationalID);
    request.input('messagingStepNationalID', sql.VarChar, messagingStepNationalID);
    request.input('phoneNumberNationalID', sql.NVarChar, phoneNumberNationalID);
...
module.exports = {updatePensionMessagingStep, updateProductDescription, updateFname, updateLastname, updateEmail, updateNationalID, update};