...
function updateReason(statusReason, phoneNumberReason, messagingStepReason, textReason, config, textIDATReason, rate) {
  sql.connect(config, function (err) {
    ...
    const request = new sql.Request();
    const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isRating', messagingStep= '3', ratingReason = @textReason WHERE phoneNumber = @phoneNumberReason AND text_id_AT = @textIDATReason AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberReason)`;
    ...
    request.input('ratingReason', sql.NVarChar, ratingReason);
    request.query(updateAccounts, function (err, results) {
      ...
      request.query("SELECT TOP 1 * FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberReason AND status = @statusReason1 AND isActive = 1 AND text_id_AT = @textIDATperiodName1 order by time DESC", function (err, periodNameResults) {
        if (err) {
          console.error('Error executing query: ' + err.stack);
          return;
        }
