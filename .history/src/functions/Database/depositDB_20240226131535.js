...
function  updatePassword(phoneNumberPassword, textPassword, textIDATPassword, sender, config, textIDAT, sms, deposit, LinkID) {
...
function  updateDescription(phoneNumberDescription, textDescription, textIDATDescription, sender, config, textIDAT, sms, deposit, LinkID, memberID) {
  sql.connect(config, function (err) {
    ...
    const request = new sql.Request();
    const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isDeposit', messagingStep= '3', description = @textDescription, memberID = @memberID WHERE phoneNumber = @phoneNumberDescription AND text_id_AT = @textIDATDescription AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberDescription)`;
...
function updateAmount(sender, statusAmount, phoneNumberAmount, messagingStepAmount, textAmount, config, textIDATAmount, textIDAT, sms, LinkID) {
  sql.connect(config, function (err) {
    ...
    request.query(updateAccounts1, function (err, results) {
      ...
      request2.query("SELECT TOP 1 m.*, t.* FROM two_way_sms_tb t JOIN members_tb m ON CAST(SUBSTRING(t.memberID, CHARINDEX('.', t.memberID) + 1, LEN(t.memberID)) AS INT) = m.m_id WHERE t.phoneNumber = @phoneNumberAmount AND t.status = @statusServices AND t.isActive = 1 AND t.text_id_AT = @textIDAT ORDER BY t.time DESC;", function (err, descriptionResults) {
        if (err) {
          console.error('Error executing query: ' + err.stack);
          return;
        }
        if (descriptionResults.recordset.length > 0) {
          ...
          const amount = parseInt(textAmount, 10);
          console.log(amount);
          ...
          var args = {
            data: { accountName: accountName, amount: amount, phoneNumber: phoneNumber },
            headers: { "Content-Type": "application/json" }
          };
          console.log(args);
          makeDeposit.post("https://api.octagonafrica.com/v1/contributions/lipanampesa", args, function (data, response) {
            if ([200].includes(response.statusCode)) {
              ...
              sql.connect(config, function (err) {
                ...
                request.query(updateAllPeriods, function (err, results) {
                  ...
                  console.log('Making Deposit attempt successful');
                  sql.close();
                });
              });
              sms.sendPremium({
                to: sender,
                from: '24123',
                message: "Dear "+ user_full_names + ",  please check your phone for the STK Push to complete the transaction",
                bulkSMSMode: 0, 
                keyword: 'pension',
                linkId: LinkID
              });
            ...
            sql.connect(config, function (err) {
              ...
              sql.connect(config, function (err) {
                ...
                console.log('Connected to the database');
                const request = new sql.Request();
                const statuserror404 = "DepositFailed";
                const messagingSteperror404 = "0";
                const phoneNumbererror404 = sender;
                const textIDATerror404 = textIDAT;
                ...
                request.query(updateDelete, function (err, results) {
                  ...
                  console.log(' Make Deposit Attempt unsuccessful');
                  sql.close();
                });
              });
            }else if ([500].includes(response.statusCode)) {
