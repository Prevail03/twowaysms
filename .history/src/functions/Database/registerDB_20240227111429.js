        ...
        if (registerResults.recordset.length > 0) {
          ...
          if ([200].includes(response.statusCode)) {
            ...
            sql.connect(config, function (err) {
              ...
              const updateError = `UPDATE two_way_sms_tb SET status = @statuserror500, messagingStep = @messagingSteperror500, isActive = 0  WHERE phoneNumber = @phoneNumbererror500 AND text_id_AT =@textIDATerror500 AND time = (
              ...
              request.query(updateError, function (err, results) {
                if (err) {
                  console.error('Error executing query: ' + err.stack);
                  return;
                }
