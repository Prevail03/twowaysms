...
function updatePensionMessagingStep(statusIsPension, phoneNumberRateValue, messagingRateValue, textRateValue, config, textIDATRateValue)
updatePensionMessagingStep(statusIsPension, phoneNumberPension, messagingStepPension, textMessageProduct, config, textIDATPension); {
  sql.connect(config, function (err) {
    ...
    const updateDelete = `UPDATE two_way_sms_tb SET status = @statusIsPension, messagingStep = @messagingRateValue, rateValue = @textRateValue WHERE phoneNumber = @phoneNumberRateValue AND text_id_AT = @textIDATRateValue AND time = (
