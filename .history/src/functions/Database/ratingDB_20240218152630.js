...
function updateService(statusService, phoneNumberService, messagingStepService, textService, config, textIDATService) {
  ...
  sql.connect(config, function (err) {
    const request = new sql.Request();
    const updateService = `UPDATE two_way_sms_tb SET status = @statusService, messagingStep = @messagingStepService, ratingService = @textService WHERE phoneNumber = @phoneNumberService AND text_id_AT = @textIDATService AND time = (
  SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberService )`;
    ...
    request.query(updateService, function (err, results) {
      if (err) {
        console.error('Error executing query: ' + err.stack);
        return;
      }
