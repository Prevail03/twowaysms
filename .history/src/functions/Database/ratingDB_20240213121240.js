          ...
          addNewUserRating.post("https://api.octagonafrica.com/v1/adduserratings", args, function (data, response) {
            ...
            if ([200].includes(response.statusCode)) {
              ...
              sql.connect(config, function (err) {
                ...
                request.input('phoneNumberReason', sql.NVarChar, phoneNumberReason);
                request.input('textIDATperiodID', sql.NVarChar, textIDAT);
                ...
                request.query(updateAccounts, function (err, results) {
                  ...
                  request2.input('phoneNumberReason', sql.NVarChar(50), phoneNumberReason);
                  request2.input('textIDATperiodID1', sql.NVarChar(50), textIDATperiodID1);

                  request2.query("SELECT TOP 1 * FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberReason AND status = @statusperiodID AND isActive = 1 AND text_id_AT = @textIDATperiodID1 order by time DESC", function (err, periodIDResults) {
                    if (err) {
                      console.error('Error executing query: ' + err.stack);
                      sql.close();
                      return;
