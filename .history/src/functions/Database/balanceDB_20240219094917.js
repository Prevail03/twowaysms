...
function updatePassword(phoneNumberPassword, textPassword, textIDATPassword, sender, config, textIDAT, sms, LinkID) {
  sql.connect(config, function (err) {
    ...
    const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isBalance', messagingStep= '1', password = @textPassword WHERE phoneNumber = @phoneNumberPassword AND text_id_AT = @textIDATPassword AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberPassword)`;
    ...
    request.query(updateAccounts, function (err, results) {
      ...
      const statusPassword = "isBalance";
              ...
              accountIDClient.get("https://api.octagonafrica.com/v1/accountsid", args, function (data, response) {
                if (response.statusCode === 200) {
                  ...
                  sql.connect(config, function (err) {
                    ...
                    console.log('Connected to the database');
                    const request = new sql.Request();
                    const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isBalance', messagingStep= '2', user_id = @textUserID WHERE phoneNumber = @phoneNumberUserID AND text_id_AT = @textIDATuserID AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberUserID)`;
                    ...
                    request.query(updateAccounts, function (err, results) {
                      ...
                      const statusUserIDRequest = "isBalance";
                            ...
                            if ([200].includes(response.statusCode)) {
                              ...
                              if (total_accounts === 0) {
                                ...
                                sql.connect(config, function (err) {
                                  console.log('Connected to the database');
                                  const request = new sql.Request();
                                  const statusAccountsEntry = "isBalance";
                ...
                checkIfExistsRequestSysUsers.query(checkIfExistsQuerySysUsers, function (checkErrSysUsers, checkResultsSysUsers) {
                  ...
                  if (checkResultsSysUsers.recordset.length > 0) {
                    ...
                    if (loginAttempts <= 2) {
                      ...
                      const request = new sql.Request();
                      const statuserror404 = "isBalance";
...
function updateDescription(phoneNumberDescription, textDescription, textIDATDescription, sender, config, textIDAT, sms, LinkID, memberID) {
  sql.connect(config, function (err) {
    if (err) {
      console.error('Error connecting to the database: ' + err.stack);
      return;
    ...
    const request = new sql.Request();
    const updateAccounts = `UPDATE two_way_sms_tb SET status = 'isBalance', messagingStep= '5', description = @textDescription, memberID = @memberID WHERE phoneNumber = @phoneNumberDescription AND text_id_AT = @textIDATDescription AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberDescription)`;
    ...
    request.query(updateAccounts, function (err, results) {
      ...
      const statusDescription = "isBalance";
          ...
          getBalance.get("https://api.octagonafrica.com/v1/accounts/balance", args, function (data, response) {
            if ([200].includes(response.statusCode)) {
              ...
              sql.connect(config, function (err) {
                ...
                const updateAllPeriods = `UPDATE two_way_sms_tb SET status = @statusPeriodsEntry    WHERE phoneNumber = @phoneNumberPeriodsEntry AND text_id_AT =@textIDATPeriodsEntry AND time = (
