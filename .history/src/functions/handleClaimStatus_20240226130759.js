...
function handleBalanceEnquiry(textMessage, sender, messagingStep, sms, config, textIDAT, LinkID, claimStatus, account){
  switch (parseInt(messagingStep)) {
    ...
    case 2:
      ...
      const checkIfExistsQuery = "SELECT TOP 1 * FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberDescription AND isActive = 1 AND status = 'isCheckingClaim' AND time = (SELECT MAX(time) FROM two_way_sms_tb WHERE phoneNumber = @phoneNumberDescription)";
      ...
      checkIfExistsRequest.query(checkIfExistsQuery, function(checkErr, checkResults) {
          ...
          if (checkResults.recordset.length > 0) {
              ...
              if (accountDescription >= 1 && accountDescription <= accountsArray.length) {
                  ...
                  console.log("Account Description Claim Status: " + textDescription);
                  ...
                  sms.sendPremium(claimStatus.invalidResponse(sender, LinkID));
              }

          } else {
          console.log('Record does not exist');
          sms.sendPremium(claimStatus.invalidResponseSystem(sender, LinkID));
          }
      });
      });
    break;
