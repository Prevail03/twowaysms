            ...
            checkIfExistsRequest.query(checkIfExistsQuery, function(checkErr, checkResults) {
                ...
                if (checkResults.recordset.length > 0) {
                    ...
                    //     sms.sendPremium(register.wrongMenuValue(sender, LinkID));
                        ...
                        const request = new sql.Request(connection);
                        const updateDeposit = `UPDATE two_way_sms_tb SET status = @statusDeposit, isActive=@isActive, messagingStep = @messagingStepBalance WHERE phoneNumber = @phoneNumberBalance AND time = (
